...
def get_rough_aircraft_cost(MTOW, SHP_TO):
    ...
    return C_aircraft, C_engines, C_airframe

MTOW = 15000
SHP_TO = 3000

C_aircraft, C_engine, C_airframe = get_rough_aircraft_cost(MTOW, SHP_TO)
...
def RAND_DAPCAP_IV(W_e, V, Q):
    ...
    RDTE_flyaway = (HE * RE) + (HT * RT) + (HM * RM) + (HQ * RQ) + CD + CF + CM # + (C_eng * N_eng) + C_avionics
    return RDTE_flyaway

RDTE_flyaway = RAND_DAPCAP_IV(10000, )

# ------------------------- Operation and Maintenance Costs ------------------------- #

def operation_maintenance(t_b, CEF, W_A, R, RL):
